...
class AVClassifier_Sum(nn.Module):
    ...
    def __init__(self,args):
        ...
        self.audio_net = resnet18(modality='audio')
        self.visual_net = resnet18(modality='visual')
        self.audio_cls = nn.Linear(512,309)
        self.visual_cls = nn.Linear(512,309)
...
class AV_Early_Classifier(nn.Module):
    ...
    def __init__(self,args):
        ...
        self.audio_net = resnet18(modality='audio')
        self.visual_net = resnet18(modality='visual')
        self.mm_encoder = MaxOut_MLP(309,2048,1024,1024,linear_layer=False)
        self.head = MLP(1024,128,309,one_layer=True)
    def forward(self,audio,visual,pad_audio=False,pad_visual=False):
        if pad_audio:
            audio = torch.zeros_like(audio,device=audio.device)
